...
<div class="p-8 space-y-10">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('dekanat.inventaris.create') }}" class="text-white bg-primary-hover px-4 py-2.5 text-xs font-bold rounded-lg shadow-sm hover:bg-primary transition flex items-center gap-2">
    ...
    <form action="{{ route('dekanat.inventaris.index') }}" method="GET" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <x-container>
            <x-table
                :headers="['NO', 'Gambar', 'Nama Barang', 'Kategori', 'aktif', 'tidak aktif', 'aksi']"
                :cols="['60px', '1fr', '1.5fr', '1fr', '1fr', '1fr', '140px']"
                :data="$inventaris"
                    ...
                    <x-table-row>
                        ...
                        <div class="justify-center flex gap-2">
                            <x-action-button 
                                ...
                                title="Lihat Data" 
                                as="a" 
                                href="{{ route('dekanat.inventaris.show', ['inventaris' => $item->id, 'status' => $status]) }}"
                            />
                            <x-action-button
                                type="delete"
                                onclick="event.preventDefault(); openDeleteModal('{{ route('dekanat.inventaris.destroy', $item->id) }}')"
                            />
                        </div>
                    </x-table-row>
                @empty
...
<x-popup-del
    ...
    message="Apakah Anda yakin ingin menghapus inventaris ini?<br>Seluruh stok dan riwayat terkait juga akan dihapus.<br><strong>Tindakan ini tidak dapat dibatalkan.</strong>"
/>
