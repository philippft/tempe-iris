...
@php
    // Menentukan subtitle secara otomatis berdasarkan role dan identifier
    $subtitle = match($role) {
        'admin_dekanat'    => $identifier,
        'admin_lm'         => $identifier,
        'mahasiswa'        => $identifier,
        'petinggi_dekanat' => strtoupper($identifier), // misal: WAKIL DEKAN III
        default            => ''
    };

    // Daftar Menu dan Hak Akses Role
    $menus = [
        [
            'label'  => 'Dashboard',
            'route'  => 'dashboard', // Sesuaikan dengan nama route Anda
            ...
            @if (in_array($role, $menu['roles']))
                ...
                >
                    ...
                    <span class="w-6 h-6 shrink-0 transition-colors duration-200 
                        {{ $isActive ? 'text-primary-hover' : 'text-[#64748B] group-hover:text-primary-hover' }}">
